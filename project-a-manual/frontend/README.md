...
- **No automated frontend tests yet** — the backend has unit + BDD coverage (see [backend/README.md](../backend/README.md#testing)); the frontend currently has none. The MVVM split is what would make that cheap to add (Models and ViewModels are plain functions/hooks, testable without a DOM), but no test runner (Jest/Vitest/Playwright) is wired up yet.
